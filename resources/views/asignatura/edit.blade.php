<x-app-layout>
    ...
    @if ($datos['modo'] == "Cerrar") 
        <x-authentication-card>
            ...
            <form method="POST" action="{{ route('asignatura.update', ['id' => $datos['id'], 'modo' => 'Cerrar']) }}">
                @csrf
                @method('PUT')
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Estas a punto de cerrar {{$datos['nombre']}} </h2>
                <p>Las evaluaciones que hayan sido calificadas se sumaran todas y se aplicara la puntuacion para los estudiantes presentes.</p>
